    ...
    <div class="col-md-12">
        <div class="portlet box green-haze">
            <div class="portlet-title">
                <div class="caption"><i class="fa fa-cogs"></i>产品：${(product.modelName)!}  配置列表</div>
                <div class="actions">
                    <div class="btn-group">
                        <div class="fileinput fileinput-new" data-provides="fileinput">
                            <a class="btn green btn-parent" href="${rc.contextPath}/itemMode/create-${(product.id)!}">
                                <i class="fa fa-plus"></i>
                                <span class="hidden-480">新增配置</span>
                            </a>
    ...
    <script type="text/javascript">
        ...
        grid.init({
            ...
            dataTable: {
                ...
                "iDisplayLength": 10,
                "bServerSide": true,
                "sAjaxSource": "${rc.contextPath}/itemMode/list-${(product.id)!}",
                "aaSorting": [
                    [0, "desc"]
                ],
                "aoColumnDefs": [
                ...
                "aoColumns": [
                    {"sTitle": "产品名称", "mData": "modeNo", "mRender": function (data, type, row) {
                            return "${(product.modelName)!}";
                    }},
                    {"sTitle": "配置编码", "mData": "modeNo"},
                    {"sTitle": "配置名称", "mData": "modeName"},
                    {"sTitle": "配置别名", "mData": "bname"},
                    ...
                    {"sTitle": "创建人", "mData": "createdBy"},
                    {
                        ...
                        var a = '<a href="${rc.contextPath}/itemMode/update-${(product.id)!}/' + full.id
                                + '" class="btn btn-xs blue"  title="编辑" >' +
                                '<i class="glyphicon glyphicon-pencil"></i>编辑</a>';

                        var b ='<a href="${rc.contextPath}/itemPic/index-3-' + full.id
                                + '" class="btn btn-xs green"  title="配置图片" >' +
                                '<i class="glyphicon glyphicon-picture"></i>配置图片</a>';
                        var c = '<a href="javascript:void(0);" onclick="deleteOne(\'' + full.id
                                + '\')" class="btn btn-xs red"  title="删除" >' +
                                '<i class="glyphicon glyphicon-trash"></i>删除</a>';
                        var d = '<a href="${rc.contextPath}/itemColor/index-${(product.id)!}-' + full.id
                                + '" class="btn btn-xs blue"  title="颜色信息" >' +
                                '<i class="glyphicon glyphicon-plus"></i>颜色信息</a>';

                        var e = '<a href="${rc.contextPath}/itemMode/update-${(product.id)!}/' + full.id
                                + '" class="btn btn-xs grey"  title="编辑" >' +
                                '<i class="glyphicon glyphicon-pencil"></i>编辑</a>';

                        <#--var f ='<a href="${rc.contextPath}/itemPic/index-3-' + full.id-->
                                <#--+ '" class="btn btn-xs grey"  title="配置图片" >' +-->
                                <#--'<i class="glyphicon glyphicon-picture"></i>配置图片</a>';-->
                        var g = '<a href="javascript:void(0);" onclick="deleteOne(\'' + full.id
                                + '\')" class="btn btn-xs grey"  title="删除" >' +
                                '<i class="glyphicon glyphicon-trash"></i>删除</a>';
                        var h = '<a href="${rc.contextPath}/itemColor/index-${(product.id)!}-' + full.id
                                + '" class="btn btn-xs grey"  title="颜色信息" >' +
                                '<i class="glyphicon glyphicon-plus"></i>颜色信息</a>';
                ...
                buttons: {
                    main: {
                        ...
                        callback: function () {
                            ...
                            $.ajax({
                                url: '${rc.contextPath}/itemMode/deleteOne-' + id,
                                type: 'POST',
                                traditional: true,
