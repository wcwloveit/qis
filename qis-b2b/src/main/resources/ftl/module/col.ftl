        ...
        <div class="row">
            <div class="col-md-12">
                <div class="portlet light bordered">
                    <div class="portlet-title">
                        <div class="caption">
                            <i class="fa fa-square-o"></i>数据列列表
                        </div>
                    </div>
                    <div class="portlet-body form">
                        <form class="form-horizontal" action="${rc.contextPath}/moduleColumnDatas/save" method="POST"
                              id="moduleForm">
                                ...
                                <div class="row ">
                                    ...
                                    <#--style="display: none"-->
                                        <div class="form-group columnDatas" style="display: none">
                                            <label class="control-label col-md-3">数据列</label>
                                            <div class="col-md-9">
                                            <#list columnDatas as columnData>
                                                <input type="checkbox" value="${columnData.id}" name="cols">${columnData.name}
                                            </#list>
                                            </div>
                                        </div>
                                    </div>
...
<content tag="script">
    ...
    <script type="text/javascript">
        ...
        $("#moduleTree").jstree({
            ...
            if (module_id) {
                $.ajax({
                    url: '${rc.contextPath}/moduleColumnDatas/getCols/' + module_id,
                    type: 'GET',
                    success: function (msg) {
                        ...
                        $("input[name=cols]").removeProp("checked");
                        $("input[name=cols]").parents('span').removeClass("checked");
                        if(cols){
                            for(var i=0;i<cols.length;i++){
                                var col=cols[i];
                                $("input[name=cols][value='" +col.columnDataId+"']").prop("checked", true).parents('span').toggleClass("checked");
                            }
                        }
